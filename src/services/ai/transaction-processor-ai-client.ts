...
import type { AIServiceClient, BaseAIService } from './interfaces'
...
const logger = createLogger('TransactionProcessorAIClient')

/**
 * AI client implementation for processing receipts and extracting transaction data
 */
export class TransactionProcessorAIClient extends BaseAIClient implements AIServiceClient {
  /**
   * Initialize the transaction processor AI client
  ...
  constructor(
    ...
    super(apiKey, model, baseUrl, refererUrl)
    
    if (!prompt) {
      logger.warn('Transaction processor initialized without a prompt template')
    }
  }
